################################################################################
# Utility module for AWS Glue ETL operations (IPORT Pipeline)
#
# Covers:
...
def read_json_from_s3(env):
    ...
    Config file path pattern:
        s3://application-payload-institutional-<env>-us-east-1/
    ...
    try:
        ...
        bucket_name = "application-payload-institutional-" + env_name + "-us-east-1"
...
def get_dat_file_row_count(env, file_name):
    ...
    try:
        if env == 'eng':
            bucket_name = "institutional-sftp-eng-us-east-1-internal-bucket-ily-eng"
            file_path   = "home/aidmq-sftp-sat/" + file_name
        elif env == 'test':
            bucket_name = "institutional-sftp-test-us-east-1-internal-bucket-ily-test"
            file_path   = "home/aidmq-sftp-sat/" + file_name
        elif env == 'prod':
            bucket_name = "institutional-sftp-prod-us-east-1-internal-bucket-ily-prd"
            file_path   = "home/aidmq-sftp-sat/" + file_name
        else:
...
def check_file_exists(file_name, env):
    ...
    """
    if env == 'eng':
        bucket_name = "institutional-eng-us-east-1-ily-etl-s3-source"
        file_path   = "Incoming/" + file_name
    elif env == 'test':
        bucket_name = "institutional-test-us-east-1-ily-etl-s3-source"
        file_path   = "Incoming/" + file_name
    elif env == 'prod':
        bucket_name = "institutional-prod-us-east-1-ily-etl-s3-source"
        file_path   = "Incoming/" + file_name
    else:
...
def upload_file_to_s3(env, sourcebucket):
    ...
    Uses AWS CLI with KMS server-side encryption (required per client data security policy).
    ...
    try:
        if env in ('eng', 'test', 'prod'):
            cmd = (
                ...
                f"--recursive --sse aws:kms --sse-kms-key-id alias/CLIENT-KMS-S3"
            )
        else:
            raise ValueError("Invalid environment.")
...
def file_move_archive_folder(file_name, env):
    ...
    try:
        if env == 'eng':
            bucket_name  = "institutional-eng-us-east-1-ily-etl-s3-source"
            ssekms_key_id = "arn:aws:kms:us-east-1:<account-id-eng>:key/<kms-key-id-eng>"
        elif env == 'test':
            bucket_name  = "institutional-test-us-east-1-ily-etl-s3-source"
            ssekms_key_id = "arn:aws:kms:us-east-1:<account-id-test>:key/<kms-key-id-test>"
        elif env == 'prod':
            bucket_name  = "institutional-prod-us-east-1-ily-etl-s3-source"
            ssekms_key_id = "arn:aws:kms:us-east-1:<account-id-prod>:key/<kms-key-id-prod>"
        else:
...
def execute_s3_sql_file(env, file_name):
    ...
    try:
        if env == 'eng':
            bucket_name = "application-payload-institutional-eng-us-east-1"
        elif env == 'test':
            bucket_name = "application-payload-institutional-test-us-east-1"
        elif env == 'prod':
            bucket_name = "application-payload-institutional-prod-us-east-1"
        else:
            raise ValueError("Invalid environment.")
