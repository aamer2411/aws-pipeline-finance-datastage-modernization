# Project Details — AWS DataStage Modernization, Vanguard IIG
...
![IPORT Dependent Job Execution Flow](screenshots/iport_dependent_job_execution_flow.png)

IPORT is Vanguard's institutional plan sponsor business reporting tool. It processes data across four reporting cycles: Yearly (Y), Quarterly (Q), Monthly (M), and Daily (D), organized into 7 builds.
...
SELECT * FROM iportadm.TCOMM_SEG@idmsat10.vanguard.com;
...
- AWS CLI access requires Access Key ID, Secret Access Key, and a Session Token (rotated via the client's internal Antiphony tool).
